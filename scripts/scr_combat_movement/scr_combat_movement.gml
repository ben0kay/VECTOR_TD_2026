...
/// @description Creates a reusable cached combat line-of-sight runtime.

function scr_combat_line_of_sight_cache_create(
    _interval_minimum = 8,
...
/// @description Returns a cached combat line-of-sight result.

function scr_combat_line_of_sight_cache_update(
    _source,
    _target,
    _cache,
...
)
{
    ...
    var _source_moved =
        (
            (_source_move_x * _source_move_x)
            + (_source_move_y * _source_move_y)
        )
        >= _distance_squared;
    ...
    // The configured function decides what blocks this particular beam.
    //
    // Current example:
    // scr_world_line_blocked_by_dead
    //
    // FUTURE:
    // scr_world_line_blocked_by_building
    // scr_world_line_blocked_by_any_solid
    // scr_world_line_blocked_by_shield
    ...
    _cache.timer =
        irandom_range(
            _cache.interval_minimum,
            _cache.interval_maximum
        )
        * max(
            1,
