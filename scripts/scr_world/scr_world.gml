...
/// @description Returns whether a circle overlaps an active building.

function scr_world_circle_building_solid(
    _world_x,
    _world_y,
    _radius,
    _walls_block = true
)
{
    ...
    for (var i = 0; i < _building_count; ++i)
    {
        ...
        if (!_walls_block && _building.object_index == o_wall)
            continue;
...
/// @description Returns whether a circle overlaps the CPU.

function scr_world_circle_cpu_solid(
    _world_x,
    _world_y,
    _radius
)
{
    ...
    <= _radius + _cpu.visual.radius;
}


/// @description Returns whether a circle overlaps any requested solid.

function scr_world_circle_gameplay_solid(
    _world_x,
    _world_y,
    _radius,
    _walls_block = true
...
{
    // Dead and resource terrain are permanent solid rock.
    ...
    if (
        scr_world_circle_building_solid(
            _world_x,
            _world_y,
            _radius,
            _walls_block
